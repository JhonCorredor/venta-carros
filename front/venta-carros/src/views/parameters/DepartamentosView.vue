<template>
    <div class="container">
        <div class="card text-white bg-success mb-3" style="max-width: 20rem;">
            <div class="card-header">Registro Departamentos</div>
        </div>
        ...
        <div class="form-group">
            ...
            <label class="form-label mt-1">Nombre de la ciudad</label>
            <input type="text" class="form-control" v-model="descripcion" placeholder="Ingresar nombre del departamento">


            <label class="form-label mt-1">Paises</label>
            <select class="form-select" v-model="paisId">
                <option disabled :selected="true" value="">-- Seleccione --</option>
                <option v-for="item in listPaises" :key="item.id" :value="item.id">{{item.descripcion}}</option>                
            </select>
...
export default {
    ...
    methods: {
        loadData: function () {
            axios.get('http://132.145.204.101:8009/api/departamentos').then(result => {
                this.listData = result.data
            })
            axios.get('http://132.145.204.101:8009/api/paises').then(result => {
                this.listPaises = result.data
            })
        },
        findByid: function (id) {
            // metodo para consutlar por el ig del boton impreso en la vista
            axios.get('http://132.145.204.101:8009/api/departamentos/' + id).then(result => {
                this.id = result.data.id;
                this.codigo = result.data.codigo;
                this.descripcion = result.data.descripcion;
                this.paisId = result.data.paisId.id;
                this.estado = (result.data.estado == true ? 1 : 0);
            })
        },
        deleteById: function (id) {
            // metodo para eliminar el registro por ID
            axios.delete('http://132.145.204.101:8009/api/departamentos/' + id).then(() => {
                Swal.fire({
                        icon: 'success',
        ...
        dataAdd: function () {
            ...
            axios.post('http://132.145.204.101:8009/api/departamentos', data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
        ...
        dataUpdate: function () {
            ...
            axios.put('http://132.145.204.101:8009/api/departamentos/' + this.id, data).then(result => {
                if (result.data) {
                    Swal.fire({
                        icon: 'success',
                        title: "'El registro se guardó de forma correcta.'",
